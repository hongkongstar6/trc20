...
| `etc/tls/`            | sign 服务 mTLS 证书目录（仓库里只有占位文件，证书自己放）                                                        |
...
6. **拿能量**：`sweep_server.energy_rental=true` 走 `AcquireRented`（比价/按序选平台，不退化烧 TRX）→ 等委托到账（按下单前的基线能量确认）→ 状态改 `energy_ready`，记录费用模式与订单号；租不到就结束本次归集并打 error。
   `sweep_server.energy_rental=false`（留空则跟随 `energy.rental_enabled`）时不下单，先检查地址 TRX 够不够烧手续费，不够就本轮跳过、打 error、不写记录，也不会改成租赁。
...
`POST /v1/withdraw` → `createWithdraw`：校验目标地址合法、`notify_url` 是绝对 http(s)、金额为币种本位的正数（`amount="11"` 就是 11 USDT，小数位不超过币种精度，入库前按精度换算成最小单位）、链/币种受支持；写 `withdraw_record`（`status=created`，`from_address=热钱包`，我方生成 `trade_no`）。
...
4. **能量**：`EstimateEnergy` 估算 → 有能量池且够用就直接用；否则 `AcquireRented` 只租不烧（租不到就 `halt` 告警，绝不烧热钱包 TRX）。
   `withdraw_server.energy_rental=false`（留空则跟随 `energy.rental_enabled`）时改为检查热钱包 TRX 是否够烧手续费，不够同样 `halt` 并打 error，不会改成租赁。
